...
export class LensImpl<T, U> {
  ...
  // public fromPath<K1 extends keyof U>([path1]: [K1]) {
  //   return this.k(path1)
  // }
  //
  // public fromPath<K1 extends keyof U, K2 extends keyof U[K1]>([path1, path2]: [K1, K2]) {
  //   return this.k(path1)
  // }

  public lensFromPath(path: Array<any>): any {
    const lens = this.k(path[0])
    return path.slice(1).reduce((acc, prop) => acc.k(prop), lens)
  }
